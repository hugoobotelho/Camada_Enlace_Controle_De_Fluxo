...
public class Receptor {
  ...
  public void resetFrameExpected() {
    indiceNextFrame = 0;
  }
  ...
  public void CamadaEnlaceDadosReceptoraDesenquadramento(int quadro[]) {
    ...
    sequencia = retiraNumDeSerieDoQuadro(quadro);
  ...
  public int[] CamadaEnlaceDadosReceptoraJanelaDeslizanteUmBit(int quadro[]) {
    String numSequenciaQuadro = sequencia; //numero de sequencia do quadro atual
    if (numSequenciaQuadro.charAt(1) != frame_expected[indiceNextFrame].charAt(1)) { //se o numero de serie do quadro recebido nao for igual ao numero de serie do quadro esperado, ignora e manda o ack desse numero
      //System.out.println("Quadro duplicado");
      //envia o ack dele
      CamadaDeAplicacaoTransmissora(numSequenciaQuadro);
      return null;
    } else {
      //envia o ack dele
      nextFrameExpected(); //atualiza a janela
      CamadaDeAplicacaoTransmissora(numSequenciaQuadro);
      return quadro;
    }
  ...
  // ********************************************************************************************
  // ****************************** PARTE TRANSMISSORA DO RECEPTOR ******************************
  // ********************************************************************************************
    ...
    public void /* int[] */ CamadaEnlaceDadosTransmissoraEnquadramentoViolacaoDaCamadaFisica(int quadro[])
      ...
      // return quadro;
      // Chama o controle de Erro
      qtdBitsTotaisAck = qtdCaracters * 8;
    ...
    public void CamadaDeEnlaceTransmissoraControleDeErro(int[] quadroEnquadrado) throws InterruptedException {
      ...
      //  System.out.println("Essa e o ACK");
      ...
      if (tipoDeEnquadramento != 3) { // se o enquadramento nao for violacao de camada, envia para a camada fisica
        ...
        Principal.meioDeComunicao.setQtdBitsTotaisAck(qtdBitsTotaisAck);
        Principal.meioDeComunicao.setSentidoOnda(1);
        qtdBitsTotaisAck = 0;
    ...
    public int[] CamadaEnlaceDadosTransmissoraControleDeErroBitParidadePar(int quadro[]) {
      ...
      int indexQuadro = 0;
      int qtdBitsUm = 0;
      for (int i = 0; i < qtdBitsTotaisAck; i++) { // percorre a quantidade de bits totais do quadro
        int bit = (quadro[indexQuadro] >> deslocaQuadro) & 1;
    ...
    public int[] CamadaEnlaceDadosTransmissoraControleDeErroCRC(int quadro[]) {
      ...
      qtdBitsTotaisAck += 32; // acrescenta 32 bits pq o crc tem 33 bits
    ...
    public int[] CamadaEnlaceDadosTransmissoraControleDeErroCodigoDeHamming(int quadro[]) {
      ...
      qtdBitsTotaisAck += 7; // adiciona 7 bits que sao os 7 bits de contorle
    ...
    public void CamadaFisicaTransmissora(int quadro[]) throws InterruptedException {
      ...
      switch (tipoDeDecodificacao) {
        case 0: // codificao binaria
          // qtdBitsTotaisAck = qtdBitsTotaisAck + (8 * qtdCaracters);
          CamadaFisicaTransmissoraCodificacaoBinaria(quadro);
          break;
        case 1: // codificacao manchester
          // qtdBitsTotaisAck = qtdBitsTotaisAck + (8 * qtdCaracters);
          CamadaFisicaTransmissoraCodificacaoManchester(quadro);
          // se o tipo do enquadramento for violacao, o calculo da qtdBitsTotaisAck muda pois
          // quando adiciona informacao de controle, ela nao dobra pois nao passa pela
          // codificacao mancherter
          /*
           * if (tipoDeEnquadramento == 3) {
           * qtdBitsTotaisAck = 2 * qtdBitsTotaisAck - (qtdBitsTotaisAck - qtdCaracters * 8);
           * } else {
           * qtdBitsTotaisAck = 2 * qtdBitsTotaisAck;
           * }
           */
          break;
        case 2: // codificacao manchester diferencial
    ...
    public void CamadaFisicaTransmissoraCodificacaoBinaria(int quadro[]) {
      // implementacao do algoritmo
      // System.out.println("Esse e o quadro do ACK");
      // for (int i = 0; i < quadro.length; i++){
      //  System.out.println(String.format("%32s",
      //  Integer.toBinaryString(quadro[i])).replace(' ', '0'));
      // }
    ...
    public void CamadaFisicaTransmissoraCodificacaoManchester(int quadro[]) throws InterruptedException {
      ...
      // }
      // System.out.println("Essa e a quantidade de bits totais: " + qtdBitsTotaisAck);
